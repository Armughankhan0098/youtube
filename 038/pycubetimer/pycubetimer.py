...
def freeze(args=0):
	...
	if (count < 12):
		while (i <= count):
			if (i == count):
				saved_times_str += times_list[count - i].strip()
				break
	...
	if (count >= 12):
		...
		while (p < 12):
			...
			if (times_list_12_flo[p] == min_12 and min_c12 == 0):
				...
				min_c12 = 1
				continue
			avg12_flo += times_list_12_flo[p] / 10
			p += 1
		avg12 = '%.2f' % (avg12_flo)
	...
	mean = 0
	while (k < count):
		mean += float(times_list[k].strip()) / count
		k += 1
	mean = '%.2f' % (mean)
...
saved_times = Label(
    root,
    text=f'Last 12 Solves: {saved_times_str}',
    font=20,
    background='#f6e58d',
    fg='black',
)
